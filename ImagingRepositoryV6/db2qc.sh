...
mysql --batch --raw --silent -h172.16.10.80 -ucody -p4cody2c ImagingRepositoryV6 -e "call getStudyPaths('-nt -s -d -sq +n $study');" | sed -e 's#__#_#g' -e 's#,##g' -e 's#\t#\ #' | sort -R | while read -a t
do
	id=${t[0]}
	sess=${t[1]}
	path=${t[2]}
	...
	if [ -f $repo_root/TRK/$id/dti.trk ]; then
