...
while getopts "i:f:x:vmo" opt
do
 case $opt in
  i) sid="$OPTARG";;
  f) fsd="$OPTARG";;
  x) export FREESURFER_VERSION="/$OPTARG";;
  v) export VIEWER=1;;
...
if [ ${fsd:-0} -eq 0 -o ${#fsd} -eq 0 ]; then
	fsd=($(mysql --login-path=cody -Brs ImagingRepositoryV6 -e "select StudyImageID from StudyImageInfo where ModalityID=2 and ScanSessionID=(select ScanSessionID from StudyImageInfo where StudyImageID=${sid});"))
	[ ${#fsd[@]} -gt 1 ] && echo "Multiple structural associated with this scan: ${fsd[@]}" && exit
fi

[ ${OVERWRITE:-0} -eq 0 -a -f /data/DWW/$sid/count_0.txt ] && echo "$sid already processed" && exit
[ ! -d /data/FS/${fsd:-0}${FREESURFER_VERSION} ] && echo "Freesurfer parcellation missing: $fsd" && exit
...
echo $sid $fsd $FREESURFER_VERSION >> $d/dww.log
...
[ -f $d/aparc.2009.nii.gz      ] || mri_convert /data/FS/${fsd}${FREESURFER_VERSION}/mri/aparc.a2009s+aseg.mgz $d/aparc.2009.nii.gz &


#[ -f $d/brain.nii.gz         ] || mri_convert /data/FS/$fsd/mri/brain.mgz $d/brain.nii.gz
#[ -f $d/brain.nii.gz         ] || exit
[ -f $d/mprage_fs_seg_2.nii.gz ] || mri_convert /data/FS/${fsd}${FREESURFER_VERSION}/mri/wm.seg.mgz $d/mprage_fs_seg_2.nii.gz &  #fast -n 3 -t 1 -g -b -B -o $d/mprage_fs $d/brain &
